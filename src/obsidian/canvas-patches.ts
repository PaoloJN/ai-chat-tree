...
// TODO: remove empty node creation logic

export const calcEmptyHeight = (options: { parentHeight: number; text: string }) => {
	const calcTextHeight = Math.round(
		textPaddingHeight +
		(pxPerLine * options.text.length) / (minWidth / pxPerChar)
	)
...
export const createEmptyNode = (
	...
	const height =
		...
		Math.max(
			minHeight,
			parentNode && calcEmptyHeight({ text, parentHeight: parentNode.height })
		)
